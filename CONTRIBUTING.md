...
- E2E smoke (**Playwright**, headless Chromium in CI):
  ...
  - Local debugging: in `frontend/`, `npm run e2e:smoke:ui` / `npm run e2e:smoke:debug` (smoke only) or `npm run e2e:ui` / `npm run e2e:debug` (all specs); often with `PW_NO_WEBSERVER=1` and servers up. After a local run, `npm run e2e:report` opens the HTML report (`playwright-report/`).
