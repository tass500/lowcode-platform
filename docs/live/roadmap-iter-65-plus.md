...
1. **Integrációs minőség** — további `Backend.Tests` lefedettség kritikus API-kon (auth / BFF / workflow), a **63b** vonal természetes folytatása — [`roadmap-next-iterations.md`](roadmap-next-iterations.md). *Folyamatban:* `GET /api/auth/spa-oidc-config` — `AuthSpaOidcConfigTests`.  
