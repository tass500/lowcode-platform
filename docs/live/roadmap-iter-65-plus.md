...
2. **Supply-chain / security CI (iter 66+)** — NuGet audit + vulnerable check — [`ci-supply-chain.md`](ci-supply-chain.md); titok-szűrés — Gitleaks — [`ci-secret-scanning.md`](ci-secret-scanning.md); backend + frontend **SBOM** (CycloneDX artefaktok) — [`ci-sbom.md`](ci-sbom.md); opcionális: GitHub Dependency graph + dependency-review; további: npm audit kötelező kapu (Angular upgrade után) — [`roadmap-iter-64-plus.md`](roadmap-iter-64-plus.md).  
