...
- **Backend (ASP.NET Core)**
  ...
  - **CI supply-chain (66+):** `NuGetAudit` a `backend/Directory.Build.props`-ban; `backend-supply-chain` job — `dotnet list … package --vulnerable` (GitHub **Dependency review** action opcionális, ha a repóban engedélyezve van a Dependency graph) — [`ci-supply-chain.md`](ci-supply-chain.md).
