...
- **Backend (ASP.NET Core)**
  ...
  - **Integrációs tesztek (folyamatos):** **68a–b** — `WorkflowRunEndpointsTests` — tenant **`GET /api/workflows/runs`**: szűrők, lapozás, **UTC** kötelező a dátum query-khez (időzóna nélküli string → 400), `startedBeforeUtc` érvényes eset — [`roadmap-iter-68-quality.md`](roadmap-iter-68-quality.md). **68c** — `EntityDefinitionEndpointsTests` — **`GET /api/entities`**: név szerinti sorrend, üres lista + **`serverTimeUtc`** — ugyanott. **68d** — `EntityRecordEndpointsTests` — **`GET /api/entities/{id}/records`**: 404 ismeretlen entitásnál, üres lista + **`serverTimeUtc`**, frissítés utáni sorrend — ugyanott. `AuthSpaOidcConfigTests` — `GET /api/auth/spa-oidc-config` (hiányos vs. kitöltött OIDC + SPA kliens) — [`oidc-jwt-bearer.md`](oidc-jwt-bearer.md); `AdminObservabilityTests` — `GET /api/admin/observability` (admin JWT + telepítés sor); `AdminUpgradeRunsTests` — `GET /api/admin/upgrade-runs/recent|latest|queue` + `take` validáció + 401; `AdminUpgradeRunsMutationTests` — `POST` start / cancel, `GET` by id, enforcement **hard block**; `AdminInstallationTests` — `GET /api/admin/installation/status`; `AdminAuditTests` — `GET /api/admin/audit` + `export`; `AdminTenantsTests` — `GET /api/admin/tenants` + `POST` slug validáció — [`tenant-api-key.md`](tenant-api-key.md); `BffAuthEndpointsTests` / `BffSessionBearerWorkflowTests` — BFF meta, login, callback, session — [`security-audit-logging.md`](security-audit-logging.md) / [`ops/upgrade.md`](ops/upgrade.md) / [`auth-bff-httponly.md`](auth-bff-httponly.md) kontextus.
