...
- **Backend (ASP.NET Core)**
  ...
  - **CI supply-chain (66+):** `NuGetAudit` a `backend/Directory.Build.props`-ban; `backend-supply-chain` job — `dotnet list … package --vulnerable` (GitHub **Dependency review** action opcionális, ha a repóban engedélyezve van a Dependency graph); **Dependabot** csoportosítás (kevesebb heti PR) — [`ci-supply-chain.md`](ci-supply-chain.md).
  - **CI titok-szűrés (66+):** külön workflow — **Gitleaks** (`gitleaks.yml`), `.gitleaks.toml` allowlist teszt/dev placeholderre — [`ci-secret-scanning.md`](ci-secret-scanning.md).
  - **CI SBOM (66+):** `CI` workflow — **CycloneDX JSON** backendre (`LowCodePlatform.Backend.csproj`, `.config/dotnet-tools.json` / cyclonedx) + frontendre (`npm sbom --package-lock-only`); artefaktok: `sbom-backend-cyclonedx-json`, `sbom-frontend-cyclonedx-json` — [`ci-sbom.md`](ci-sbom.md).
  - **CI CodeQL:** külön workflow (`codeql.yml`) — **csharp** `build-mode: manual`, **javascript-typescript** `none`; konfig — [`ci-codeql.md`](ci-codeql.md).
  ...
  - **Integrációs tesztek (folyamatos):** **68a–b, 68h** — `WorkflowRunEndpointsTests` — futások **lista** + **`domainCommand`** hibakódok a run-on — [`roadmap-iter-68-quality.md`](roadmap-iter-68-quality.md). **68i** — `WorkflowEndpointsTests` — **`GET /api/workflows`** (név szerinti sorrend, üres lista + **`serverTimeUtc`**) — ugyanott. **68j** — ugyanott — **`PUT .../schedule`**: 404, **`schedule_cron_missing`** — ugyanott. **68k** — ugyanott — **`POST /import`** (format / `definition_missing`) + **`DELETE`** 404 — ugyanott. **68c, 68f** — `EntityDefinitionEndpointsTests` — entitás **lista** + **név** validáció / ütközés (**409**), **GET** 404 — ugyanott. **68g** — `EntityFieldEndpointsTests` — **`/api/entities/{id}/fields`** — ugyanott. **68d–e** — `EntityRecordEndpointsTests` — rekord **lista** + **POST/GET** validáció és hibakódok — ugyanott. `AuthSpaOidcConfigTests` — `GET /api/auth/spa-oidc-config` (hiányos vs. kitöltött OIDC + SPA kliens) — [`oidc-jwt-bearer.md`](oidc-jwt-bearer.md); `AdminObservabilityTests` — `GET /api/admin/observability` (admin JWT + telepítés sor); `AdminUpgradeRunsTests` — `GET /api/admin/upgrade-runs/recent|latest|queue` + `take` validáció + 401; `AdminUpgradeRunsMutationTests` — `POST` start / cancel, `GET` by id, enforcement **hard block**; `AdminInstallationTests` — `GET /api/admin/installation/status`; `AdminAuditTests` — `GET /api/admin/audit` + `export`; `AdminTenantsTests` — `GET /api/admin/tenants` + `POST` slug validáció — [`tenant-api-key.md`](tenant-api-key.md); `BffAuthEndpointsTests` / `BffSessionBearerWorkflowTests` — BFF meta, login, callback, session — [`security-audit-logging.md`](security-audit-logging.md) / [`ops/upgrade.md`](ops/upgrade.md) / [`auth-bff-httponly.md`](auth-bff-httponly.md) kontextus.
...
- **Frontend (Angular)**
  - **E2E smoke (Playwright):** `frontend/e2e/smoke.spec.ts` — fő low-code + **New workflow/entity**, **Admin tenants**, **Upgrade**, **Auth** + **`/lowcode/auth/callback`** + részletes útvonalak (workflow / run / entity / records, nem létező UUID-val); CI: `scripts/e2e-smoke-ci.sh` → `npm run e2e:smoke` + **`frontend-e2e`**; lokális Windows: `scripts/e2e-smoke-ci.ps1`; közreműködői útmutató: [`CONTRIBUTING.md`](../../CONTRIBUTING.md), [`frontend/README.md`](../../frontend/README.md) — [`e2e-smoke-plan.md`](e2e-smoke-plan.md).
