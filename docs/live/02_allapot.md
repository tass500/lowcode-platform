...
- **Backend (ASP.NET Core)**
  ...
  - **CI supply-chain (66+):** `NuGetAudit` a `backend/Directory.Build.props`-ban; `backend-supply-chain` job — `dotnet list package --vulnerable`; PR-ekre `dependency-review` (új/módosított függőségek) — [`ci-supply-chain.md`](ci-supply-chain.md).
