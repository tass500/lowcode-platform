...
- **Backend (ASP.NET Core)**
  ...
  - **BFF auth (iter 62c, Fázis B):** opcionális `Auth:Bff:Enabled` (+ környezeti kapu); `GET/POST /api/auth/bff/*` — login redirect, callback, Data Protection session süti, `meta` / `session` / `logout`; OIDC: `Auth:Oidc:Authority` + `SpaClientId` — [`auth-bff-httponly.md`](auth-bff-httponly.md). **Még nincs:** workflow API-k automatikus Bearer injektálása a sütiből (Fázis C).
